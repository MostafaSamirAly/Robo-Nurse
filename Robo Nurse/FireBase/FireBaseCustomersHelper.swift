...
class FireBaseRoomsHelper {
    ...
    private let customersRef = Database.database().reference().child("customers")
    private init() {}
    ...
    func checkUserExistance(completion:@escaping (Result<Bool,Error>)->Void) {
        ...
        customersRef.child(uid).observeSingleEvent(of: .value){ (snapshot) in
            if snapshot.exists() {
                ...
                if let payment = userDict["payment"] as? String {
                    UserDefaultsHelper.shared.setPaid(true)
                    UserDefaultsHelper.shared.setSubscription(type: payment)
                }
